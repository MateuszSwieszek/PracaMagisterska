...
from tensorflow.keras.layers import BatchNormalization, Conv2D, Conv2DTranspose, MaxPooling2D, Dropout, UpSampling2D, \
    Input, concatenate, Flatten, Dense, Activation, Reshape
...
figpath = f'/net/people/plgmswieszek/GenerowanieObrazkow/Callbacks/Plots/segm_ALL_' + str(cv) + '.png'
figpathJson = f'/net/people/plgmswieszek/GenerowanieObrazkow/Callbacks/JsonFiles/segm_ALL_' + str(cv) + '.json'
...
for n, (inp, labIm) in enumerate(zip(x_test, y_test)):
    predicted = pred[n]>0.5
    inp = inp>0.5
    dice = Dice(predicted, labIm)
    dice2 = Dice(predicted, inp)
    dice3 = Dice(inp, labIm)
