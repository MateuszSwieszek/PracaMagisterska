...
def PCATransform(points, stdPoints, homographyPoints, images):
    ...
    for p in range(a.shape[0]):
        im[int(a[p, 0]), int(a[p, 1]), 0] = 200
        im[int(a[p, 0]), int(a[p, 1]), 1] = 0
        im[int(a[p, 0]), int(a[p, 1]), 2] = 0
    ...
    for p in range(X_org.shape[0]):
        im[int(X_org[p, 0]), int(X_org[p, 1]), 0] = 0
        im[int(X_org[p, 0]), int(X_org[p, 1]), 1] = 0
        im[int(X_org[p, 0]), int(X_org[p, 1]), 2] = 139

    # showImage(im)
    # plt.imshow(im, cmap='gray')
    # plt.savefig('zaznaczone_'+name.split('/')[-1].split('.')[0]+'.jpg')
    cv2.imwrite('zaznaczone_'+name.split('/')[-1].split('.')[0]+'.png', im)

    print("Rozmiar obrazu:", im[9,9])
    # img = CImg(im)
    # img.display()

    print("calculate diff: ", X_org.shape)
    mean_diff = print(np.mean(np.sqrt(np.sum(X_org - a, -1) * np.sum(X_org - a, -1))))
    print(mean_diff)
...
def main():
    ...
    stdPoints = WczytywaniePunktowSTD(
        POINTSDIR1='/Users/mateusz/PycharmProjects/PracaMagisterska/Orgs/Points/stdPoints/',
    ...
    homographyPoints = WczytywaniePunktowHomography(
        POINTSDIR1='/Users/mateusz/PycharmProjects/PracaMagisterska/Orgs/Points/homographyPoints/',
